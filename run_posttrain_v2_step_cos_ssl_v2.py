#!/usr/bin/env python3
# =========================
# SFT Post-train v2 — Cosine-decay continuation from v3 (step_const) checkpoints
#
# Continuation of the constant-LR SFT run in sft_posttrain_v3 for
# sft_step in [SFT_RESUME_STEP, SFT_TARGET_STEPS]. Applies a cosine-annealing
# schedule over that window so the model can "settle" onto the plateau
# instead of hovering.
#
# Behaviour:
#   - Seeds the new run from SFT_PRIME_DIR/.../step{SFT_RESUME_STEP:06d}.pt
#     (copied into SFT_OUT_DIR once, so original v3 is untouched).
#   - Also copies the v3 CSV log rows with sft_step <= SFT_RESUME_STEP so
#     the new CSV log contains a continuous trajectory.
#   - CosineAnnealingLR over SFT_TARGET_STEPS - SFT_RESUME_STEP steps
#     (no warmup, since the optimizer state is already stable at peak LR).
#   - Eval + checkpoint every SFT_EVAL_EVERY_STEPS.
# =========================
...
# Prime from the constant-LR v3 runs; write cos-decay continuation here.
SFT_PRIME_DIR = "./sft_posttrain_v3"
SFT_OUT_DIR = "./sft_posttrain_v2"

# SSL steps to use for SFT — per-seq_len mapping.
# Only 100k SSL ckpts are used for this experiment (best on dev search).
...
# All 8 folds (2015-2017 .. 2015-2024).
FOLD_FILTER = [
...
# Continuation: resume from sft_step=SFT_RESUME_STEP and run cosine decay until SFT_TARGET_STEPS.
SFT_RESUME_STEP = 150_000
SFT_TARGET_STEPS = 200_000
SFT_EVAL_EVERY_STEPS = 5_000
SFT_BATCH_SIZE = 4096
# Peak LR for cosine decay window (set to the LR used when step_v2 ran, i.e. constant LR).
# CosineAnnealingLR will take these as the starting point and decay to SFT_LR_MIN.
SFT_LR_ENCODER = 5e-6
SFT_LR_HEAD = 1.5e-5
SFT_LR_MIN = 1e-7
...
def run_posttrain_one_task(
    ...
    cos_total_steps = int(SFT_TARGET_STEPS - SFT_RESUME_STEP)
    if cos_total_steps <= 0:
        raise RuntimeError(
            f"SFT_TARGET_STEPS ({SFT_TARGET_STEPS}) must be > SFT_RESUME_STEP ({SFT_RESUME_STEP})"
        )
    scheduler = CosineAnnealingLR(opt, T_max=cos_total_steps, eta_min=SFT_LR_MIN)

    n_batches_sft = (len(train_ds) + SFT_BATCH_SIZE - 1) // SFT_BATCH_SIZE
    _log(
        f"SFT cosine-decay continuation: resume_step={SFT_RESUME_STEP}, target={SFT_TARGET_STEPS} "
        f"(cos T_max={cos_total_steps}, eta_min={SFT_LR_MIN}), eval every {SFT_EVAL_EVERY_STEPS} steps "
    ...
    #   - the primer step{SFT_RESUME_STEP:06d}.pt (and make it last.pt)
    #   - the CSV log up to sft_step == SFT_RESUME_STEP
    if not os.path.exists(last_path):
        prime_dir = os.path.join(
            SFT_PRIME_DIR, f"seqlen_{seq_len}", fold_name, f"seed{seed}",
            f"ssl_step{ssl_step:06d}",
        )
        primer_ckpt = os.path.join(prime_dir, f"step{SFT_RESUME_STEP:06d}.pt")
        if not os.path.exists(primer_ckpt):
            raise RuntimeError(
                f"Primer checkpoint missing: {primer_ckpt}. "
                f"Make sure step_v2 has finished sft_step={SFT_RESUME_STEP} for this task."
            )
        _log(f"[PRIME] seeding run_dir from {prime_dir}")
        _log(f"[PRIME] copying {primer_ckpt} -> {last_path}")
        shutil.copy2(primer_ckpt, last_path)

        # Copy CSV rows up to (and including) SFT_RESUME_STEP so the new CSV
        # contains a continuous trajectory from step=SFT_EVAL_EVERY_STEPS..SFT_TARGET_STEPS.
                ...
                with open(primer_csv, "r", encoding="utf-8") as fin:
                    ...
                    for line in fin:
                        ...
                        try:
                            ...
                            continue
                        if s <= SFT_RESUME_STEP:
                            kept.append(line)
                with open(step_csv, "w", encoding="utf-8") as fout:
                    fout.writelines(kept)
                _log(f"[PRIME] copied {len(kept)-1} CSV rows (sft_step <= {SFT_RESUME_STEP})")
            except Exception as e:
    ...
    if os.path.exists(resume_path):
        ...
        scheduler = CosineAnnealingLR(opt, T_max=cos_total_steps, eta_min=SFT_LR_MIN)
        for _ in range(max(0, int(sft_step - SFT_RESUME_STEP))):
            scheduler.step()
        cur_lrs = [g["lr"] for g in opt.param_groups]
        _log(f"[RESUME] cosine scheduler advanced by {max(0, sft_step-SFT_RESUME_STEP)} steps, "
             f"current LRs: enc={cur_lrs[0]:.3e} revin={cur_lrs[1]:.3e} "
             f"head={cur_lrs[2]:.3e} gate={cur_lrs[3]:.3e}")
    ...
    while sft_step < SFT_TARGET_STEPS:
        ...
        for indices in batch_indices_list:
            ...
            # Advance cosine decay (only in the continuation window [RESUME, TARGET]).
            if sft_step >= SFT_RESUME_STEP:
                scheduler.step()
...
def main():
    ...
    master_log(
        f"SFT_RESUME_STEP={SFT_RESUME_STEP} SFT_TARGET_STEPS={SFT_TARGET_STEPS} "
    ...
    master_log(
        f"PRIME from {SFT_PRIME_DIR} (step{SFT_RESUME_STEP:06d}.pt)  ->  OUT {SFT_OUT_DIR}"
    )
    master_log(
    ...
    master_log(
        f"Differential LR cosine decay: peak enc={SFT_LR_ENCODER}, peak head={SFT_LR_HEAD}, "
        f"eta_min={SFT_LR_MIN}, T_max={SFT_TARGET_STEPS - SFT_RESUME_STEP}, WD={SFT_WEIGHT_DECAY}"
    )
