...
def ssl_pretrain_worker(task: Dict) -> Dict:
    ...
    for ep in range(start_ep, SSL_MAX_EPOCHS + 1):
        ...
        # Pre-generate ALL time masks for this epoch on CPU, move to GPU once
        rng = np.random.RandomState(seed + ep * 1000003)
        all_time_masks_np = make_time_span_mask_fast(
            B=len(train_ds),
            L=seq_len,
            ...
            rng=rng,
            pad_lens=train_ds.pad_lens_np,
        )
        all_time_masks_gpu = torch.from_numpy(all_time_masks_np).to(device)
        del all_time_masks_np

        # Flat shuffle: mix samples across all days, split into real batch_size chunks
        ...
        for indices in batch_indices_list:
            ...
            time_mask = all_time_masks_gpu[indices]
            B, L, _F = x.shape
        ...
        del all_time_masks_gpu, batch_indices_list
